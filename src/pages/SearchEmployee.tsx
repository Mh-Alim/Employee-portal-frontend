  ...
  return (
    <div className=" bg-slate-200  h-[100vh] w-full flex justify-center items-center  ">
      <main className=" m-3  w-11/12 md:w-9/12 lg:w-7/12 bg-white rounded-lg   p-5 ">
        {/* search */}
        <form onSubmit={submitHandler} className=" flex items-center bg-white p-1 rounded-tl-md rounded-tr-md border-b-slate-200 border-b-2  ">
        ...
        <div className=" border-b-2 border-slate-200 overflow-scroll flex w-full bg-white  pt-3 pl-3  ">
          <Category name="Interest" count={5} idx={1} border={border} setBorder={setBorder} />
          <Category name="Name" count={23} idx={2} border={border} setBorder={setBorder} />
          <Category name="Email" count={2} idx={3} border={border} setBorder={setBorder} />
          <Category name="Communities" count={2} idx={4} border={border} setBorder={setBorder} />
...
const Category = ({ name, count,idx, border,setBorder }: CategoriesType) => {
  return (
    <span onClick={() => {setBorder(idx)}} className={`  cursor-pointer transition-all  duration-500 flex justify-center items-center text-center min-w-32  py-2 border-b-2 ${border === idx ? `border-black` : `border-white`}   `}>
      {name}
